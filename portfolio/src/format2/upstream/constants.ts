...
import { Database, BarChart3, Code2, Server, Globe, Terminal, LucideIcon, LineChart, ShoppingBag, Wallet, Cpu, Cloud, Layout, Shield, Wrench } from 'lucide-react';
...
/** Case-insensitive icon for resume @tech labels (e.g. "Tools", "Languages"). */
export function categoryIconForLabel(label: string): LucideIcon {
  const trimmed = String(label || "").trim();
  if (!trimmed) return Terminal;
  const hit = Object.keys(CATEGORY_ICONS).find(
    (k) => k.toLowerCase() === trimmed.toLowerCase(),
  );
  if (hit) return CATEGORY_ICONS[hit];
  const lower = trimmed.toLowerCase();
  if (lower.includes("language") || lower.includes("programming")) return Code2;
  if (lower.includes("database") || lower === "sql") return Database;
  if (
    lower.includes("bi") ||
    lower.includes("viz") ||
  ...
  if (lower.includes("cloud") || lower.includes("aws") || lower.includes("infra"))
    return Cloud;
  if (lower.includes("engineer") || lower.includes("etl") || lower.includes("pipeline"))
    return Server;
  if (lower.includes("library") || lower.includes("framework")) return Layout;
  return Terminal;
}
