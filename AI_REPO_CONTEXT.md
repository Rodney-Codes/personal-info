...
| `ui` | Merged defaults + portfolio format + frontmatter; includes `templateVariant`, `heroTagline`, `pdfButton`, `profilePhotoUrl` (optional; sync may infer from GitHub API `avatar_url` + `cb=` cache-buster from resume GitHub contact), etc. |
