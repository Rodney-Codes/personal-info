...
The portfolio **does not** call a hosted chatbot API (no Render or `VITE_CHATBOT_*` build wiring). Chatbot integration is paused until re-enabled; see **`docs/chatbot-integration.md`**.
...
|---|---|---|---|---|---|
| `resume_1` | `portfolio_2` | `resume_format_1` | `template_format_2` | Recommended | Current production/default pairing. |
...
## Chatbot (paused — local tooling only)

Portfolio AMA UI and production API hosting (formerly Render) are **disabled**. This repo keeps chatbot **content and Python tooling** for a future re-integration:

- App-specific FAQ: **`content/chatbot_faq.md`**
- Corpus / eval scripts: **`scripts/build_corpus_from_workflow.py`**, **`scripts/eval_retrieval.py`**
- In-repo API package (local dev): **`src/docs_chatbot_service/`**
- Standalone service repo: [Rodney-Codes/docs-chatbot](https://github.com/Rodney-Codes/docs-chatbot)
- Status and re-enable steps: **`docs/chatbot-integration.md`**, **`AI_README.md`**

```bash
cd portfolio && npm run sync   # site JSON + resume PDF only (no chatbot index)
```
